...
11. For Windows, cd into the edatasheets_creator directory and type:
    ...
    py main.py 
    ```
    You should see an input file not found error because no command line arguments have been specified. This is OK and confirms that the environment is set up correctly.

12. Set up a launch.json file for debug by pressing the Run and Debug button on the left nav bar in Visual Studio Code and creating a launch.json if one does not already exist.  Here is an example of a launch.json file (don't forget to change the paths to point to a sample file)
    ```json
    {
...
The e-datasheet has some fields which are required in the output file depending on the plugin being used. The following subsection show what fields will be entered in the corresponding output file for each file format.

### DITA (XML)

#### Included Fields in Output

1. namespace: Static field with the URL of Intel Design. Value: https://www.intel.com/design
1. generatedOn: Date of generation of the datasheet with format YYYY-MM-DD-HH-MM-SS.
1. generatedBy: Team that generates the datasheet. In this moment all of this files are generated by e-datasheet team, and for that reason this is a static field, Value: Intel e-Datasheet Generator.
...
2. description: Description of the contents of the datasheet.
3. inputFile: File used to generate the datasheet, includes the GUID and the extension of the file.
4. createdOn: Date of generation of the datasheet with format YYYY-MM-DD-HH-MM-SS.
5.  guid: Unique Identifier of the datasheet, needs to match the file name of the datasheet and the input file.
...
For a specific plugin to use this option, the keyword `validateInputs` needs to be set to false in the plugin_cfg.json definition for the plugin. The caseFormats also needs to be defined to specify the type and default value for each argument in the case list.

When using this functionality, ensure that the plugin class inherits from PluginBase class. This base class adds the validation schema for input arguments to the plugin. The validation uses the marshmallow library that converts complex data types to native Python data types and vice versa.
...
Once the run method is invoked by the Core module, the Runner will process the list of Jobs and Tasks that were parsed from the Pipeline file or created by Core. If there are multiple Jobs or Tasks, the Runner will iterate over them. The Pipeline.json file specifies the order of each Job and Task execution.
...
## Individual files
For the input, output, map and vocabulary parameters in the pipeline.json file, include the full path to the files that will be used and generated. As an example:
...
Only the map or vocabulary parameter require a single file to be specified. For the other parameters; arg1, arg2 and output parameters, paths to the files that will be processed and directory for the outputs to be placed need to be specified.
```json
{
  "pipelineDescription":"Automate form xls to edatasheet",
  "pipelineJobs":[
...
When validateInputs flag is set to False,  multi-argument function can be used. This allows the arguments to be of any type other than a file path. For example, in the Directory listing plugin, the validateInputs flag is set to False thus giving room for a directory as an input.
...
Depending on if the execution is for a Pipeline.job file or files indicated from the command line arguments, the system will link the corresponding plugin in the following ways:
...
## 10.Translator
This plug-in takes a ".json" input file (e-datasheet) and generates an output ".json" file translated in other language defined by the user.
...
5. setup() method call: This method is used to specify the application name, version, author, option and executables for the executable being built.
